...
devpulse/
├── api/
│   └── index.js                  # Root serverless entrypoint for Vercel
├── backend/
│   ├── api/
...
├── package.json                  # Root-level package descriptor for Vercel
...
4. **Optimized API Responses**: The GitHub proxy route strips the raw GitHub API response down to only the fields the UI needs (`name`, `description`, `url`, `stars`, `language`, `updatedAt`), cutting payload size significantly versus the full repo object.
